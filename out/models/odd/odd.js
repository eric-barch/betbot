...
class Odds {
    ...
    initialize() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const exchangeId = exchange.getSqlExchange().get('id');
            const gameId = game.getSqlGame().get('id');
            const awayTeamId = awayTeam.getSqlTeam().get('id');
            const homeTeamId = homeTeam.getSqlTeam().get('id');
            const spreadOdds = this.getSpreadOdds();
            const moneyOdds = this.getMoneyOdds();
            const overUnderOdds = this.getOverUnderOdds();
            yield databaseModels.Odd.findOrCreate({
                ...
                defaults: {
                    spreadAwaySpread: spreadOdds.getAwaySpread(),
                    spreadHomeSpread: spreadOdds.getHomeSpread(),
                    spreadAwayPrice: spreadOdds.getAwayPrice(),
                    spreadHomePrice: spreadOdds.getHomePrice(),
                    moneyAwayPrice: moneyOdds.getAwayPrice(),
                    moneyHomePrice: moneyOdds.getHomePrice(),
                    totalOverUnder: overUnderOdds.getOverUnder(),
                    totalOverPrice: overUnderOdds.getOverPrice(),
                    totalUnderPrice: overUnderOdds.getUnderPrice(),
                    exchangeId: exchangeId,
                    gameId: gameId,
