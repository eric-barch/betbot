import fs from 'fs';
import verb from './verbosity.json' assert { type: 'json' };

const verbosity = verb._config.verbosity['generator.js'];

function getFileStructure(path) {
    const verbose = verbosity.getFileStructure;
...
function getComponentStructure(path) {
    const verbose = verbosity.getComponentStructure;
...
function getClasses(fileContents) {
    const verbose = verbosity.getClasses;
...
function getClassIndices(fileContents) {
    const verbose = verbosity.getClassIndices;
...
function getClosingBraceIndex(str, startIndex) {
    const verbose = verbosity.getClosingBraceIndex;
...
function getFunctions(fileContents) {
    const verbose = verbosity.getFunctions;
...
const readPath = '/Users/ericbarch/Documents/Development/AutomaticSportsBetting/iteration-5/src';
const writePath = '/Users/ericbarch/Documents/Development/AutomaticSportsBetting/iteration-5/src/_config/verbosity/verbosity.json';

const json = getFileStructure(readPath);
const writeData = JSON.stringify(json, null, 2);
