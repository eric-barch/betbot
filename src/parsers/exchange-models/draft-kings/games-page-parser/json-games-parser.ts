...
export class JsonGamesParser {
  ...
  private async getJsonGames(): Promise<Array<any>> {
    ...
    const gameScriptElements = await this.gamesPageParser.page.$$(
  ...
  private async parseJsonGamesToSequelizeGames({
    ...
    for (const jsonGame of jsonGames) {
      const awayTeam = await this.parseJsonTeamToSequelizeTeam({
        jsonTeam: jsonGame.awayTeam,
      });
      const homeTeam = await this.parseJsonTeamToSequelizeTeam({
        jsonTeam: jsonGame.homeTeam,
      });
