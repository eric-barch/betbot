...
export class FanDuelNbaGamesPageParser extends baseModels.GamesPageParser {
    ...
    private async getGameTeams(gameLinkElement: p.ElementHandle<HTMLAnchorElement>): Promise<{
        ...
        const awayTeam = await db.models.Team.findByUnformattedName({ unformattedName: awayTeamNameFull });
        const homeTeam = await db.models.Team.findByUnformattedName({ unformattedName: homeTeamNameFull });
